...
const zhHant = {
	...
	home: {
		...
		whyTitle: "為什麼做這個",
		whyBody:
			"一人公司最常見的死因是收不到錢，以及被行政瑣事吃掉本來該用來接案的時間。報價寄出去沒人追、貨出了不知道客戶還欠多少、申報期限到了才開始翻單據，光靠認真一點解決不了，需要系統替你記得。",
		...
		trust: {
			...
			cards: [
				...
				{
					title: "你自己驗得到",
					a: "操作紀錄頁直接顯示驗證結果，以及最近一次錨定的時間與它是否真的送到了外部。這件事的重點就是你不必相信我的說法。",
					strong: "",
					b: "",
				},
			],
		...
		sections: [
			{
				...
				title: "報價與訂單",
				intro:
					"報價單與訂單各自有一套狀態機。哪些狀態能轉到哪些狀態是寫死的規則，非法的轉移會被後端擋下。例如已出貨的訂單不能作廢，因為庫存已經動了，那要用沖銷，單據不會消失。",
				items: [
			...
			},
			{
				...
				items: [
					{ title: "流水帳為準", body: "進貨、出貨、調整都是一筆異動紀錄，append-only，寫錯用反向沖銷，原本的紀錄不動。" },
			...
			},
			{
				...
				items: [
					{ title: "稅別分開計算", body: "應稅、零稅率、免稅依單據明細的稅別快照分別彙總，各有自己的小計。" },
	...
	pricing: {
		...
		faqs: [
			...
			{
				q: "可以把資料帶走嗎？",
				a: "營業稅 401 的申報資料可以匯出 CSV（需要方案包含營業稅申報功能）。其餘資料目前沒有一鍵整包匯出的功能，這是我知道的缺口，之後會補上。操作紀錄可以在畫面上查閱與篩選。",
			},
			{
				q: "會計師要另外算一個人嗎？",
...
const en: CooContent = {
	...
	home: {
		...
		quote: {
			...
			ticks: [
				...
				{ a: "You're reminded before it expires, and a daily background job marks expired quotes, so there is no status to maintain by hand", b: "", c: "" },
			],
			mockAria:
		...
		trust: {
			...
			cards: [
				...
				},
				{
					...
					a: "The activity log shows the verification result directly, along with when the last anchor was made and whether it actually reached the outside. The whole point is that you don't have to take my word for it.",
					strong: "",
					b: "",
				},
			],
		...
		overview: [
			...
			{ title: "Working with your accountant", body: "Invite your accountant so they never need your password. Roles: owner, accountant, read-only." },
			...
			{ title: "Mail service", body: "Guided setup for mail on your own domain, plus forwarding rules (a third-party provider; I don't run mail servers)." },
		...
		sections: [
			{
				...
				intro:
					"Quotes and orders each have their own state machine. Which status may move to which is fixed in code, and illegal transitions are rejected by the backend. A shipped order can't be voided, for instance, because stock has already moved; that calls for a reversal, and the document stays.",
				items: [
			...
			},
			{
				...
				intro:
					"Integrated with ECPay for online payments and e-invoicing. Payment callbacks verify the signature and also re-check the amount against the local order before the status changes; verifying the signature alone means trusting the numbers the other side sent. (The integration follows ECPay's public documentation and has not been verified with real transactions in the sandbox.)",
				items: [
			...
			},
			{
				...
				items: [
					{ title: "Tax categories kept apart", body: "Taxable, zero-rated and exempt are totalled separately from the tax-category snapshot on each line, each with its own subtotal." },
			...
			},
			{
				...
				title: "Collaboration",
				intro:
					"The usual approach at filing time is to give the accountant your login. That hands over every permission along with the credibility of your audit trail, and after that no entry can say who did what.",
				items: [
			...
			},
			{
				...
				items: [
					...
					{ title: "Mail service", body: "Guided setup to connect your own domain to a third-party mail provider (Cloudflare, Resend, Postmark) and record forwarding rules. I deliberately do not run a mail server of my own. That means IP reputation and bounce complaints, wildly out of proportion for a one-person company. Sending and receiving stay with the provider." },
					{ title: "In-app notifications", body: "Anything you need to know, such as a quote about to expire or an administrator accessing your data, stays in the system with a timestamp and a read state." },
				],
			},
		],
	...
	pricing: {
		...
		compareBody: "The table below matches the plan settings in the system item by item.",
		...
		faqs: [
			{
				...
				a: "No. Billing is not live, and every newly registered company currently gets the fully featured Pro plan. I will announce it before charging starts, and nothing will be taken from your account without warning.",
			},
			{
			...
			},
			{
				...
				a: "The VAT 401 filing data exports as CSV (on plans that include VAT filing). The rest has no one-click bulk export yet. That is a gap I know about and will close. The activity log can be browsed and filtered on screen.",
			},
			{
